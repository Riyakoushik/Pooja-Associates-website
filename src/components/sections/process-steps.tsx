    ...
    <section id="how-it-works" className="py-[120px] bg-white overflow-hidden">
      <div className="container mx-auto px-6 max-w-[1200px]">
        ...
        <div className="flex flex-col items-center text-center mb-[80px]">
          ...
          <a 
            href="#contact" 
            className="group flex items-center gap-2 px-6 py-3 bg-[#1A4D43] text-white rounded-full transition-all hover:opacity-90"
          >
          ...
          <div className="space-y-[100px] relative">
            {steps.map((step) => (
              <div key={step.id} className="relative grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-24 items-center">
                ...
                {/* Number Badge (Middle) */}
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 hidden md:flex items-center justify-center w-10 h-10 rounded-full bg-[#1A4D43] text-white text-[14px] font-semibold border-4 border-white">
                  {step.id}
                </div>

                {/* Left Side (Image for 1 & 3, Text for 2) */}
                <div className={`${step.align === 'right' ? 'order-1' : 'order-1 md:order-2'} flex justify-center`}>
                  {step.align === 'right' ? (
                    <div className="relative w-full aspect-[4/3] max-w-[500px] rounded-[24px] overflow-hidden shadow-sm">
                      <Image 
                        src={step.image} 
                        alt={step.title}
                        fill
                    ...
                    <div className="w-full max-w-[450px] text-left">
                      ...
                      <button className="flex items-center gap-2 text-[#1A4D43] font-semibold transition-opacity hover:opacity-80">
                ...
                {/* Right Side (Text for 1 & 3, Image for 2) */}
                <div className={`${step.align === 'right' ? 'order-2' : 'order-2 md:order-1'} flex justify-center`}>
                  {step.align === 'right' ? (
                    <div className="w-full max-w-[450px] text-left">
                      <h3 className="text-[24px] font-semibold text-[#121212] mb-4">{step.title}</h3>
                      <p className="text-[18px] leading-[1.6] text-[#555555] mb-6">
